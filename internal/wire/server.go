...
// New builds a server over a kernel.
func New(guard auth.Guard, unguarded kernel.Kernel, log *slog.Logger) *Server {
	return &Server{guard: guard, kernel: unguarded, log: log}
}
...
func (s *Server) session(ctx context.Context) (auth.Session, error) {
...
func (s *Server) fail(err error) error {
	return fail(err, func(unexpected error) {
		s.log.Error("unexpected failure", "err", unexpected)
	})
}
