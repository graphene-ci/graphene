...
func (r *Runner) Start(ctx context.Context, spec process.Spec) (process.Started, error) {
	dir := filepath.Join(r.Root, spec.Name)
	...
	proc := &running{cmd: cmd, done: make(chan struct{}), outputs: []*os.File{stdout, stderr}}
	go proc.reap()
...
func (p *running) signal(sig syscall.Signal) {
