// Пайплайн — программа пользователя, со своим модулем и своими версиями.
// Именно так это будет выглядеть у людей: их репозиторий, их провайдеры.
module acme/vm
...
// У настоящего пользователя здесь стояла бы версия с GitHub. Пример
// проверяет SDK ИМЕННО ЭТОГО коммита, поэтому смотрит в соседний каталог.
